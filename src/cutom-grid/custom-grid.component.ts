...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [CommonModule, TableModule, NestedTableComponent],
...
export class CustomGridComponent {
  ...
  constructor() {
    this.groupedData = this.groupBy(this.originaldata, [
      'status',
      'representative.name',
      'company',
      'name',
  ...
  groupBy(data: any[], keys: string[]): any[] {
    if (keys.length === 0) {
      return data;
    }
    ...
    const groupedMap = data.reduce((acc, item) => {
      const keyValue = this.getNestedValue(item, currentKey);
      if (!acc[keyValue]) {
        acc[keyValue] = [];
      }
    ...
    return Object.keys(groupedMap).map((key) => ({
      groupName: key,
      expanded: false, // you can customize this as needed
      groupCurrentKey: currentKey,
      groupData: this.groupBy(groupedMap[key], remainingKeys),
    }));
  }
